...
class PlanetTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let theCell = tableView.dequeueReusableCell(withIdentifier: "PlanetCell", for: indexPath) as! PlanetTableViewCell
        
        let planet = planets[indexPath.row]
        theCell.planetNameLabel.text = planet.name
        ...
        theCell.numberOfMoonsLabel.text = moonsText
        theCell.numberOfFactsLabel.text = "\(planet.facts.count) facts"
        theCell.numberOfDaysFullOrbitLabel.text = "\(planet.fullOrbit) days (full orbit)"
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier != "FactTableViewController" { return }
